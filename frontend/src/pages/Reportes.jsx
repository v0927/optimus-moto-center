...
async function exportarExcel(ventasMensuales, topProductos, ventasCategoria, metricas) {
  ...
  const estiloHeader = {
    ...
    border:    { bottom: { style: 'thin', color: { argb: 'FF990000' } } },
  };
...
export default function Reportes() {
  ...
  const [metricas,        setMetricas]        = useState({
    margenBruto: 0, productoTop: '—', totalVentas: 0, totalTransacc: 0,
  });
  ...
  useEffect(() => {
    async function cargarReportes() {
      ...
      todasVentas.forEach(v => {
        (v.venta_items || []).forEach(item => {
          costoTotal += parseFloat(item.productos?.precio_compra || 0) * item.cantidad;
          ventaTotal += parseFloat(item.subtotal);
        });
      });
      const margen      = ventaTotal > 0 ? Math.round(((ventaTotal - costoTotal) / ventaTotal) * 100) : 0;
      const productoTop = top10.length > 0 ? top10[0].nombre : '—';
      setMetricas({ margenBruto: margen, productoTop, totalVentas, totalTransacc: todasVentas.length });
      setLoading(false);
    }
    cargarReportes();
  }, []);
  ...
  return (
    <div style={{ padding: esMovil ? '16px' : '24px', maxWidth:'1400px', margin:'0 auto' }}>
      ...
      <div style={{
        ...
        gap: esMovil ? '10px' : '16px', marginBottom:'24px',
      }}>
        <div className="card">
          <p style={{ fontSize:'12px', color:'var(--color-text-muted)', marginBottom:'6px' }}>Margen Bruto %</p>
          <p style={{ fontSize: esMovil ? '22px' : '28px', fontWeight:700, color:'#16A34A' }}>{metricas.margenBruto}%</p>
          <p style={{ fontSize:'11px', color:'var(--color-text-muted)', marginTop:'4px' }}>sobre ventas totales</p>
        </div>
        <div className="card">
          <p style={{ fontSize:'12px', color:'var(--color-text-muted)', marginBottom:'6px' }}>Producto Más Vendido</p>
          <p style={{ fontSize:'13px', fontWeight:700, color:'#1A56DB', lineHeight:1.3, marginTop:'4px' }}>{metricas.productoTop}</p>
        </div>
        <div className="card">
          <p style={{ fontSize:'12px', color:'var(--color-text-muted)', marginBottom:'6px' }}>Total de Ventas</p>
          <p style={{ fontSize: esMovil ? '18px' : '22px', fontWeight:700, color:'#CC0000' }}>{formatLempiras(metricas.totalVentas)}</p>
          <p style={{ fontSize:'11px', color:'var(--color-text-muted)', marginTop:'4px' }}>{metricas.totalTransacc} transacciones</p>
        </div>
        <div className="card">
          <p style={{ fontSize:'12px', color:'var(--color-text-muted)', marginBottom:'6px' }}>Promedio por Venta</p>
          <p style={{ fontSize: esMovil ? '18px' : '22px', fontWeight:700, color:'#D97706' }}>
            {metricas.totalTransacc > 0 ? formatLempiras(metricas.totalVentas / metricas.totalTransacc) : formatLempiras(0)}
          </p>
          <p style={{ fontSize:'11px', color:'var(--color-text-muted)', marginTop:'4px' }}>por transacción</p>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom:'24px' }}>
        ...
        <ResponsiveContainer width="100%" height={esMovil ? 180 : 240}>
          <LineChart data={ventasMensuales} margin={{ top:5, right:20, left:10, bottom:5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" />
            <XAxis dataKey="mes" tick={{ fontSize: esMovil ? 10 : 12, fill:'#64748B' }}
              axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: esMovil ? 9 : 11, fill:'#64748B' }}
              axisLine={false} tickLine={false}
              tickFormatter={v => `L ${(v/1000).toFixed(0)}k`} />
            <Tooltip content={TooltipLempiras} />
            <Line type="monotone" dataKey="total" stroke="#CC0000"
              strokeWidth={2.5} dot={{ fill:'#CC0000', r:4 }} activeDot={{ r:6 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="card">
          ...
          {topProductos.length === 0 ? (
            <p style={{ fontSize:'13px', color:'var(--color-text-muted)',
              textAlign:'center', padding:'24px 0' }}>Sin datos de ventas aún</p>
          ) : (
            <ResponsiveContainer width="100%" height={esMovil ? 220 : 280}>
              <BarChart data={topProductos} layout="vertical"
                margin={{ top:0, right:20, left:10, bottom:0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: esMovil ? 9 : 11, fill:'#64748B' }}
                  axisLine={false} tickLine={false}
                  tickFormatter={v => `L ${(v/1000).toFixed(0)}k`} />
                <YAxis type="category" dataKey="nombre"
                  width={esMovil ? 100 : 130}
                  tick={{ fontSize: esMovil ? 9 : 10, fill:'#64748B' }}
                  axisLine={false} tickLine={false} />
                <Tooltip content={TooltipLempiras} />
            ...
            return (
              <div key={i} style={{ marginBottom:'14px' }}>
                <div style={{ display:'flex', justifyContent:'space-between',
                  ...
                  <span style={{ color:'var(--color-text-muted)' }}>{formatLempiras(c.total)}</span>
                </div>
      ...
      <div className="card">
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse',
            fontSize: esMovil ? '12px' : '13px', minWidth: esMovil ? '400px' : 'auto' }}>
            <thead>
